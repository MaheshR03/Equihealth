...
def show_map():
    ...
    # Get user's data
    user_id = session['user_id']
    user_map_file = user_data.get(user_id, {}).get('map_file', None)
    ...
    return render_template('show-map.html', has_data=has_data, map_file=user_map_file)
...
def upload_file():
    ...
    if file.filename.endswith('.csv'):
        ...
        # Generate map based on uploaded data and store map file path
        map_file_path = generate_map(df, user_id)
        user_data[user_id]['map_file'] = map_file_path
...
def generate_map(data, user_id):
    ...
    if 'Latitude' in data.columns and 'Longitude' in data.columns:
        ...
        map_file_path = f'static/maps/map_{safe_user_id}.html'
        
        # Check if we have predictions to use for coloring
        if 'Final Prediction' in data.columns:
            # Create interactive scatter plot with predictions
            fig = px.scatter_mapbox(
                data,
                lat='Latitude',
                lon='Longitude',
                color='Final Prediction',
                hover_name='District',
                hover_data={'Final Prediction': True, 'Latitude': ':.4f', 'Longitude': ':.4f'},
                color_continuous_scale='RdYlBu_r',
                size_max=15,
                zoom=6,
                title='Health Prediction Results by District'
            )
        ...
        # Check if Target (Healthcare Access) column exists (for original functionality)
            ...
            # Create rank column
            data['rank'] = pd.cut(data['Target (Healthcare Access)'], bins=5, labels=False)
            data['rank'] = data['rank'].astype(int)
            
            # Create interactive scatter plot with ranking
            fig = px.scatter_mapbox(
                data,
                lat='Latitude',
                lon='Longitude',
                color='rank',
                hover_name='District',
                hover_data={'rank': True, 'Target (Healthcare Access)': True, 'Latitude': ':.4f', 'Longitude': ':.4f'},
                color_continuous_scale='RdYlBu_r',
                size_max=15,
                zoom=6,
                title='Healthcare Access Ranking'
            )
            ...
            # Update traces for better markers and consistent color
            fig.update_traces(marker={'size': 12, 'color': 'blue'})

        # Save the interactive map to an HTML file
        fig.write_html(map_file_path)
        
        # Return relative path for template (without 'static/' prefix)
        return f'maps/map_{safe_user_id}.html'
    else:
